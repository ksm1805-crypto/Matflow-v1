...
export const RegressionTab = ({ material, lots = [] }) => {
    ...
    const metalElements = useMemo(() => material.specification?.metalElements || STANDARD_METALS, [material]);
    const halogenKeys = ['f', 'cl', 'br'];
    ...
    const analysisData = useMemo(() => {
        ...
        return lots.map(lot => {
            ...
            // [복원] Total Impurity (Main Peak 제외)
            ...
            halogenKeys.forEach(el => dataPoint[`halo_${el}`] = parseFloat(lot.halogen?.[el] || 0));
            crossLotPeaks.forEach(peak => {
            ...
            return dataPoint;
        }).filter(d => d.lifetime > 0); // 수명 데이터 있는 것만
    ...
    const { correlations, top3Factors } = useMemo(() => {
        ...
        const calcR = (key) => {
            ...
            // 모든 값이 같으면 분산이 0이 되어 NaN 발생 -> 0 리턴
        ...
        metalElements.forEach(el => { const k = `met_${el}`; const r = calcR(k); results[k] = r; allFactors.push({ key: k, r: r, group: 'METAL', label: `Metal [${el}]` }); });
        halogenKeys.forEach(el => { const k = `halo_${el}`; const r = calcR(k); results[k] = r; allFactors.push({ key: k, r: r, group: 'HALOGEN', label: `Halogen [${el.toUpperCase()}]` }); });
        crossLotPeaks.forEach(peak => {
            if (peak.rrt < 0.95 || peak.rrt > 1.05) {
                const k = `peak_${peak.rrt.toFixed(2)}`; const r = calcR(k); results[k] = r;
    ...
    const handleCategoryChange = (newCat) => {
        ...
        else if (newCat === 'HALOGEN') setSelectedDetail(`halo_${halogenKeys[0]}`);
        else if (newCat === 'PEAK') {
    ...
    return (
        <div className="p-6 h-full flex flex-col space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-[140px]">
                        ...
                        <select className="bg-white border border-brand-200 rounded-lg px-3 py-1.5 text-sm font-bold text-brand-700 outline-none ring-1 ring-brand-100 focus:ring-brand-300" value={selectedDetail} onChange={e => setSelectedDetail(e.target.value)}>
                            ...
                            {category === 'HALOGEN' && halogenKeys.map(el => <option key={el} value={`halo_${el}`}>{el.toUpperCase()}</option>)}
             ...
             {top3Factors.length > 0 && (
                <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl p-4 text-white shadow-lg animate-in">
