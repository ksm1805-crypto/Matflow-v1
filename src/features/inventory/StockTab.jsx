...
// --- 1. Define Translation Dictionary (KO/EN/CN) ---
const TRANSLATIONS = {
    ko: {
        // Dashboard
        total_stock: "총 재고",
    ...
    en: {
        ...
        currency: "$",
        unit_large: "M", // Million (1,000,000)
        unit_small: "",
...
/**
 * StockTab.jsx
 * [Modified] Handles Product Stock (Finished Goods/Sales) exclusively.
 * [Fix] Added multi-language support (KO/EN/CN)
 */
// IMPORTANT: Receive 'lang' prop here! (Default to 'ko')
export const StockTab = ({ material, updateMaterial, readOnly, lang = 'ko' }) => {
    
    // Helper function for translations
    const t = (key) => TRANSLATIONS[lang][key] || key;

    // --- Helper Functions ---
    const updateLot = (lotId, newLotData) => { 
        if(!readOnly) updateMaterial({ ...material, lots: material.lots.map(l => l.id === lotId ? newLotData : l) }); 
    };
    
    const addShipment = (lot) => { 
        if(!readOnly) {
            updateLot(lot.id, { 
                ...lot, 
                shipments: [...(lot.shipments || []), { id: generateId(), date: new Date().toISOString().split('T')[0], customer: '', purpose: '', amount: '', unitPrice: '' }] 
            }); 
        }
    };
    
    const updateShipment = (lot, shipmentId, field, val) => { 
        if(!readOnly) {
            updateLot(lot.id, { 
                ...lot, 
                shipments: lot.shipments.map(s => s.id === shipmentId ? { ...s, [field]: val } : s) 
            }); 
        }
    };
    
    const removeShipment = (lot, shipmentId) => { 
        // Use translated confirmation message
        if(!readOnly && window.confirm(t('delete_confirm'))) {
            updateLot(lot.id, { 
                ...lot, 
                shipments: lot.shipments.filter(s => s.id !== shipmentId) 
            }); 
        }
    };

    // [Fixed Ratio] Managed at Project Level
    const fixedRatio = parseFloat(material.salesParams?.fixedRatio) || 0;
    const updateFixedRatio = (val) => { 
        if(!readOnly) updateMaterial({ ...material, salesParams: { ...material.salesParams, fixedRatio: val } }); 
    };
    ...
    const totalStats = material.lots.reduce((acc, lot) => {
        ...
        // Variable Cost (COGS) = Shipment Amount * Unit Mfg Cost
    ...
    // Dynamic Formatting for Large Numbers based on Language
    const formatBigMoney = (val) => {
        ...
        if (lang === 'en') {
            return (v / 1000000).toFixed(2); // Million
        } else {
            return (v / 100000000).toFixed(2); // 100 Million (Eok/Yi)
        }
    };

    return (
        <div className="p-6 space-y-6 h-full overflow-y-auto custom-scrollbar bg-slate-50">
                        ...
                        >
                            <div className="p-4 bg-slate-50">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    {!readOnly && (
                                        <button onClick={() => addShipment(lot)} className="text-xs bg-white border border-slate-300 hover:bg-slate-100 text-slate-600 px-2 py-1 rounded transition flex items-center gap-1 shadow-sm">
                                            <Icon name="plus" size={12}/> {t('new_ship')}
                                        </button>
                                    )}
                                </div>
