...
/**
 * StockTab.jsx
 * [수정] Material Inventory 기능 제거됨. Product Stock(완제품/매출) 기능만 전담.
 */
export const StockTab = ({ material, updateMaterial, readOnly }) => {
    ...
    const removeShipment = (lot, shipmentId) => { 
        if(!readOnly && window.confirm("Delete this shipment record?")) {
            updateLot(lot.id, { 
                ...lot, 
                        ...
                        <Card key={lot.id} title={`${lot.name} (Stock: ${fmtN(stock, 1)} g)`} icon="box" className="p-0" action={<div className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">Mfg Cost: ₩{fmtK(unitCost)}/g</div>}>
                            <div className="p-4 bg-slate-50">
                                <div className="flex justify-between items-center mb-2">
                                    <h5 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Icon name="truck" size={14}/> Shipment History</h5>
                                    {!readOnly && (
                                        <button onClick={() => addShipment(lot)} className="text-xs bg-white border border-slate-300 hover:bg-slate-100 text-slate-600 px-2 py-1 rounded transition flex items-center gap-1 shadow-sm">
                                            <Icon name="plus" size={12}/> New Shipment
                                        </button>
                                    )}
                                </div>
